...
@Component({
  selector: 'app-list-capacity-unit',
  templateUrl: './list-capacity-unit.component.html',
  styleUrls: ['./list-capacity-unit.component.scss']
})
export class ListCapacityUnitComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit() {
    ...
    this.dtOptions = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        const value: CapacityUnit = this.searchBox.value;
        that._service.datatables(value, dataTablesParameters).subscribe(resp => {
          ...
          this._toastr.error('Can\' recived the data', 'List unit of capacity');
          callback({
            recordsTotal: 0,
            recordsFiltered: 0,
            data: []
      ...
      rowCallback: (row: Node, data: CapacityUnit, index: number) => {
        $('button#action-update', row).click(() => {
          this._router.navigate(['master', 'capacity-unit', data.id]);
        });
  ...
  removed(): void {
    this._service.remove(this.idDelete).subscribe(data => {
      if (data.status === 200) {
        this._toastr.warning('Unit of capacity Removed', 'Remove Success',
          {
            timeOut: 4000
          });
        this.refresh(null);
      ...
      this._toastr.warning('Cant Delete Unit of capacity', 'Oppss...',
        {
          timeOut: 4000
        });
      console.error(error);
