...
@Component({
  selector: 'app-list-device-condition',
  templateUrl: './list-device-condition.component.html',
  styleUrls: ['./list-device-condition.component.scss']
})
export class ListDeviceConditionComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit() {
    ...
    this.dtOptions = {
      ...
      ajax: (dataTablesParameters: any, callback) => {
        const value: DeviceCondition = this.searchBox.value;
        that._service.datatables(value, dataTablesParameters).subscribe(resp => {
          ...
          this._toastr.error('Can\' recived the data', 'List device of condition');
          callback({
            recordsTotal: 0,
            recordsFiltered: 0,
            data: []
      ...
      rowCallback: (row: Node, data: DeviceCondition, index: number) => {
        $('button#action-update', row).click(() => {
          this._router.navigate(['master', 'device-condition', data.id]);
        });
  ...
  removed(): void {
    this._service.remove(this.idDelete).subscribe(data => {
      if (data.status === 200) {
        this._toastr.warning('Device of condition Removed', 'Remove Success',
          {
            timeOut: 4000
          });
        this.refresh(null);
      ...
      this._toastr.warning('Cant Delete Device of condition', 'Oppss...',
        {
          timeOut: 4000
        });
      console.error(error);
