...
@Component({
  selector: 'app-update-device-condition',
  templateUrl: './update-device-condition.component.html',
  styleUrls: ['./update-device-condition.component.scss']
})
export class UpdateDeviceConditionComponent implements OnInit, OnDestroy {
    ...
    this.subcribeTopic = this._activeRoute.params.subscribe((params: Params) => {
      ...
      this._service.getChangeTypes(id).subscribe((response: any) => {
        if (response.status === 204) {
          ...
          const value: DeviceCondition = response.body;
          console.log(value);
          this.form.setValue({
              'id': value.id,
              'name': value.name,
